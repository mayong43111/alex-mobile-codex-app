...
弹层共用紧凑标题栏、固定关闭按钮、44 像素触控控件及 16 像素表单字号；长内容在右侧滚动，数字人口播短文案表单在 320×568 屏幕直接显示制作按钮。项目、设置、服务、素材、任务、创作引导及媒体预览沿用同一套边距和字段样式，320/390 像素布局回归均通过。
...
项目首页、输入区创作引导或项目菜单的“数字人口播”提供非实时文字播报：填写最多 500 个 UTF-16 代码单元的文案，选择 Lisa 休闲/优雅坐姿及普通话晓晓/云希声音，勾选 Azure Speech 按量计费确认后制作 MP4。使用预设人物，不支持上传照片、声音克隆、实时对话或分镜剪辑；暂不提供声音试听。文案按纯文本原样发送，不执行 SSML，也不由模型改写。

首版采用独立的受控 Azure Speech batch avatar 执行适配器，不调用 Codex、不经过 OpenMontage `talking_head`，也不会启动 GPU。原有 Codex 唯一 AI 决策与 OpenMontage 图片/视频路径不变；此入口是用户明确填写参数后的机械执行，并非新增 AI 决策器。上游固定版本的 MuseTalk 执行仍未实现，自托管数字人并未安装或接通，不作为隐式回退。
...
- API 路径为 `/api/projects/:id/avatar-jobs`，沿用登录、CSRF 和项目所有者校验。全局同时最多一个未结束或待核实的数字人任务；提交前写 SQLite 账本，以请求 ID 和输入哈希去重，只发送一次生成请求。
- 刷新、断线和重启仅查询原任务。明确拒绝或云端生成失败正常标为失败，网络结果不明保留“待核实”，不自动重提、不更换服务。每 10 秒查询，满 24 小时停止查询并保留阻断；此时或云端任务已过期时，管理员须核查显示的任务 ID 和服务记录，再在停止后端后人工处理账本，不能直接清账重试。
- 输出只允许 HTTPS Azure Blob 地址，不向下载域名转发认证头或向前端暴露 SAS。最多 64 MiB；使用 `ffprobe` 校验 MP4/H.264、音轨、尺寸和 10 分钟以内的正时长，成功后原子写入任务状态与项目素材索引。素材支持授权播放、Range 和下载，无独立缩略图。生成中或待核实的任务阻止删除项目。
...
服务端仅操作该固定 VM，前端不能指定资源 ID。CLI/托管身份 token 不传给浏览器。应用存在活动或排队任务时拒绝管理操作；GPU 队列非空时禁止关闭/重启，队列读取失败时必须额外确认中断风险。已提交动作异步追踪、持久化并按请求 ID 去重；重启后继续查询，不重发 Azure 请求。Azure 明确拒绝（包括已知容量不足错误）或异步操作明确失败后保留原因并解除操作锁；界面自动核实结果，下一次确认使用新请求 ID，也可以取消此次确认。状态请求最长等待 15 秒，提交请求最长等待 30 秒；超时不等于云端取消，不自动重试。
