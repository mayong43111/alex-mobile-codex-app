...
const labels = { submitting: '正在提交', running: '正在制作', unknown: '提交待核实', completed: '已完成', failed: '生成失败' }

export function AvatarStudio({ projectId, open, onOpenChange }: { projectId: string; open: boolean; onOpenChange: (open: boolean) => void }) {
    ...
    async function refresh() {
      ...
      try {
        const [capability, latest] = await Promise.all([
          request<{ configured: boolean }>('/avatar'),
          request<AvatarJob[]>(`/projects/${projectId}/avatar-jobs`),
        ])
        if (active) { setConfigured(capability.configured); setJobs(latest); setSyncError('') }
  ...
  async function submit() {
    ...
    try {
      const job = await request<AvatarJob>(`/projects/${projectId}/avatar-jobs`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ requestId: requestId.current, text, voice, character: 'lisa', style, confirmed }) })
  ...
  return <Dialog.Root open={open} onOpenChange={onOpenChange}><Dialog.Portal>
    ...
    <Dialog.Content className="modal avatar-studio" aria-describedby={undefined}>
      ...
      <form className="avatar-form" onSubmit={event => { event.preventDefault(); void submit() }}>
        ...
        <label>数字人形象<select aria-label="数字人形象" value={style} disabled={busy} onChange={event => { setStyle(event.target.value); setConfirmed(false) }}><option value="casual-sitting">Lisa · 休闲坐姿</option><option value="graceful-sitting">Lisa · 优雅坐姿</option></select></label>
        ...
        <label className="risk-check"><input type="checkbox" checked={confirmed} disabled={busy || pending} onChange={event => setConfirmed(event.target.checked)} />确认将文案发送至 Azure Speech 并按量计费</label>
        <button type="submit" className="primary" disabled={!configured || !projectId || !text.trim() || !confirmed || busy || pending}>{busy ? <LoaderCircle size={18} className="spin" /> : <Film size={18} />}制作短片</button>
        <p className="muted" role="status">{configured === null ? '正在检查服务' : !configured ? '数字人服务未配置' : pending ? '已有任务处理中或待核实' : 'Azure Speech · MP4'}</p>
