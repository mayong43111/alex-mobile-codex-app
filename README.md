...
- Qwen 限研究评估，本轮 H3 按用户确认的社区许可与非商业测试范围使用。附件只读 MCP 工具已本地验收、尚未发布，不开放任意文件系统，不支持 PDF/Office 解析。Codex 自主决定生成或编辑，并从本轮附件和保留对话中的图片选择原图 ID，后端不再固定选上一张；上传图选择编辑已真实 Azure 验收，尚未发布。旧等待任务不会自动执行。未实现完整预算审批或中断任务自动恢复。
