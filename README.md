...
2026-09-22 已发布 `20260922-web-search-dc11780`（应用提交 `dc11780`）：按轮次展示对话、消息右下角重发/复制、图片持续加载、上一张生成图编辑、保存修复及 Codex 原生实时 Web Search。35 项测试、构建和 lint 通过，新容器启动及 Blob 探针已核实。搜索和编辑已在本地真实验证，云端托管身份下的端到端调用仍待验收；详细记录见部署文档。

# AI 图片与视频制作系统架构

更新日期：2026-09-22。状态：纯手机界面已接入本地 Codex / OpenMontage 容器，Azure GPT-5.4 对话和 image2 单图生成已真实验证。

## 当前实现

- [app/README.md](app/README.md)：本地启动、测试、存储和安全边界。已实现项目管理、需求记录、参考图上传/预览/下载、等待任务、取消和 SSE 同步。前端只面向手机，不提供 PC 布局。
- [docs/TEMP-azure-handoff.md](docs/TEMP-azure-handoff.md)：GPU 临时交接。按当前决定暂停机器申请和启动重试，先开发程序，稍后经确认再继续。A100 最近确认的状态为 deallocated，磁盘仍可能计费。
- 当前链路：手机 UI → Fastify/SQLite → 独立 Codex SDK 容器 → Azure GPT-5.4 决策 → OpenMontage BaseTool 图片适配器 → Azure gpt-image-2 → 标准检查点及项目素材库。已生成并回收一张 1024×1024 PNG，验证同一 thread 跨容器重启保持上下文。
- 根据最新决定，先使用订阅 1 现有新加坡 GPT-5.4 和用户新建的 East US2 image2 部署；本轮没有创建 Azure 资源。此本地接入安排优先于下文历史 GPU 阶段顺序及仅使用 Qwen 的目标约束。ComfyUI/Qwen 尚未接入，GPU 申请继续暂停。
- 手机端由 Codex 根据对话自动识别需求，不再手动选模式；明确出图时调用单张低质量图片工具，支持 1:1、3:2、2:3，讨论应仅对话，模糊需求应先询问。视频未接入，不以图片替代。模型意图识别仍可能出错，未实现完整预算和审批治理。右上角设置集中图片比例和费用提示。当前仅传文字，不发送参考图，调用按 Azure 实际用量计费；旧 `waiting_service` 记录不会自动执行。
- 以下章节描述目标架构，不代表全部已经落地。云端现已支持 Entra、HTTPS、PWA 和手机访问；GPU 与完整视频流水线仍未接入。
