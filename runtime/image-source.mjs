...
const sourceSchema = imageCandidateSchema.pick({ assetId: true, hash: true, width: true, height: true }).extend({ png: z.string().min(1).max(45 * 1024 * 1024) }).strict()
...
export class ImageSourceRequest {
  ...
  provide(value) {
    ...
    if (['assetId', 'hash', 'width', 'height'].some(key => source[key] !== this.candidate[key]) || !bytes.length || bytes.length > 32 * 1024 * 1024 || createHash('sha256').update(bytes).digest('hex') !== source.hash) throw new Error('Source does not match Codex selection')
