...
test('MCP tools read bounded text and images only from the run allowlist', async () => {
  ...
  const left = await sharp({ create: { width: 32, height: 32, channels: 3, background: '#ff0000' } }).png().toBuffer()
  const image = await sharp({ create: { width: 64, height: 32, channels: 3, background: '#16785d' } }).composite([{ input: left, left: 0, top: 0 }]).removeAlpha().png().toBuffer()
  const files = []
  let server, client
  try {
    ...
    assert.deepEqual((await client.listTools()).tools.map(tool => tool.name), ['list_attachments', 'read_text', 'view_image', 'view_video_frame', 'process_image'])
    ...
    const processed = await client.callTool({ name: 'process_image', arguments: { assetId: imageId, crop: { left: 32, top: 0, width: 32, height: 32 }, maxWidth: 16, maxHeight: 16 } })
    assert.notEqual(processed.isError, true)
    const result = JSON.parse(processed.content[0].text)
    assert.equal(result.width, 16)
    assert.equal(result.height, 16)
    assert.equal(processed.content[1].type, 'image')
    const [stored] = await store.processed(runId)
    assert.equal(stored.assetId, result.assetId)
    assert.equal(stored.sourceAssetId, imageId)
    const pixels = await sharp(Buffer.from(stored.png, 'base64')).raw().toBuffer()
    assert.deepEqual([...pixels.subarray(0, 3)], [22, 120, 93])
    assert.deepEqual(await readFile(store.file(runId, imageId)), image)
    assert.equal((await client.callTool({ name: 'process_image', arguments: { assetId: randomUUID() } })).isError, true)
    assert.equal((await client.callTool({ name: 'process_image', arguments: { assetId: imageId, crop: { left: 63, top: 0, width: 32, height: 32 } } })).isError, true)
    const compressed = await client.callTool({ name: 'process_image', arguments: { assetId: imageId, palette: true, quality: 60 } })
    assert.notEqual(compressed.isError, true)
    assert.equal(JSON.parse(compressed.content[0].text).width, 64)
    assert.equal(JSON.parse(compressed.content[0].text).height, 32)
    assert.equal((await client.callTool({ name: 'process_image', arguments: { assetId: imageId } })).isError, true)
...
test('stdio MCP subprocess starts with a sealed manifest and returns text on demand', async () => {
  ...
  try {
    ...
    const photo = await sharp({ create: { width: 64, height: 5024, channels: 3, background: '#16785d' } }).png({ compressionLevel: 0 }).toBuffer()
    const photoHash = createHash('sha256').update(photo).digest('hex')
    await store.stage(runId, photoId, photo, photoHash)
    const manifest = await store.seal(runId, [{ assetId, name: 'notes.txt', mediaType: 'file', mimeType: 'text/plain', bytes: bytes.length, location: `/api/assets/${assetId}/content`, sha256 }, { assetId: photoId, name: 'photo.png', mediaType: 'image', mimeType: 'image/png', bytes: photo.length, location: `/api/assets/${photoId}/content`, sha256: photoHash }])
    ...
    const processed = await client.callTool({ name: 'process_image', arguments: { assetId: photoId } })
    assert.notEqual(processed.isError, true)
    const [output] = await store.processed(runId)
    assert.equal(output.width, 64)
    assert.equal(output.height, 5024)
    assert(output.bytes < photo.length)
    assert.deepEqual(await readFile(store.file(runId, photoId)), photo)
