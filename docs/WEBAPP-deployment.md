...
2026-09-24 已将本次工作区版本发布至现有东南亚站点，镜像 `20260924-multivm-v1`，摘要 `sha256:d60c67aa9b5a5be1b0ec39aa5a7ada50eb4d6626368270f4c3c0a9ce24e9b74a`。本次代码尚未签入或推送；镜像同时包含此前 `6d5b554` 的上传取消、会话滚动、VM 恢复与文件转发改动。

- VM 面板支持共享配置列表、选择及管理员新增/编辑；普通登录用户保留状态读取与电源操作。最多 20 项配置，保存检查版本与任务状态，操作必须明确配置 ID、版本及实例名称；不提供 Azure IAM 授权。
- 配置原子持久化，旧单 VM 格式可自动迁移，操作账本按 Azure 资源标识隔离。失效实例配置仍可读取和编辑，资源不存在与 Azure 权限不足分别提示。
- 线上旧配置仍指向已删除实例，已备份并改为美国东部 2 的现有 GPU；旧操作账本保留，新资源使用独立账本。生产托管身份实际读取新 VM 状态成功，为 `running`，本次未发送任何 VM 电源动作。

45 项后端、56 项手机浏览器测试、构建及 lint 通过；生产镜像内使用 Node 原生类型剥离另通过 11 项 VM 测试。320/390 像素和宽屏截图检查通过，构建保留原有大于 500 kB 的提示。发布前后数据库完整性 `ok`、待处理任务 0、原有 3 个项目保留。应用设置、认证、托管身份和 VNet 均未变。新前端 `index-Cdc45wKx.js`、普通账号真实登录、配置 API、VM 状态及匿名 401 已核验；首次登录探测发生在冷启动期间超时，新镜像拉取后再次验证成功。

实例选择只改变管理目标，不切换生成服务的 `COMFYUI_SERVER_URL`。新 GPU 的 ComfyUI 尚未就绪；私网及生成路由切换、模型运行验收仍待后续完成。当前模型下载任务未中断，本次发布不代表 GPU 生成链路已可用。

### 移动弹层、创作引导与数字人口播

2026-09-23 已签入并推送 `1227052`，发布至现有东南亚站点。镜像 `20260923-studio-1227052`，摘要 `sha256:969e29473d434a3c2d54e8fbeb43dc6b1a45c7d18a2d36ff6cb44a9af0c52061`。

- 统一项目、设置、服务、素材、任务、创作表单与媒体预览的弹层样式；标题及关闭按钮固定，字段、操作区和滚动间距一致。数字人文案框缩为三行，320×568 首屏可见计费确认和制作按钮，保留 44 像素触控尺寸与 16 像素表单字号。
- 四项创作引导保留在首页和输入区工具栏，填写确认只追加草稿，不发起生成；修改图片要求明确来源。数字人口播同时出现在创作引导中。下载链接在浏览器需要预览时保留原页面。
- 上线 Azure Speech batch avatar 单次提交、幂等账本、权限与计费确认、重启查询恢复、MP4 校验及项目素材入库。仅为既有东南亚账户增加 Web App 身份的账户级 Speech User，配置可选端点，不配置服务密钥；未知结果不会重新生成。

37 项后端、42 项手机浏览器测试通过，构建及 lint 通过；生产容器内另通过 11 项认证/数字人测试和 ffprobe 执行检查，安装审计无已知漏洞。前端仍有既有的大于 500 kB 打包提醒。320/390 像素弹层布局及截图已检查；真实手机系统/PWA 下载交互仍待实机确认。

停站后切换镜像，保留原认证、托管身份及网络配置。首次 ARM 回读和会话响应仍来自过渡期，不能据此判断新容器接管；随后核对实际构建文件 `index-DHuInWH1.js`、`index-CtY1PuYh.css` 及新增账本表确认发布完成。04:18:37 UTC 镜像拉取完成，04:18:47 Blob 读写删除探针通过且迁移 0 文件，04:18:49 启动探针通过。发布前后数据库完整性均为 `ok`，待处理任务为 0，3 个项目保留；匿名 API（含数字人能力入口）返回 401，Entra 授权码/S256 跳转和会话入口通过。

本轮发布未发起收费生成。此前本地真实数字人口播、图片和脚本验收不等于新生产版本的登录后端到端验收。Speech 角色包含 BatchAvatar 权限，但 SCM 管理容器没有网站身份端点，无法在该容器完成托管身份的 Speech 只读调用；生产身份实际生成仍待验证。GPU 回读仍为解除分配，未删除或迁移基础设施，日本东迁移继续暂停。

### Entra 回调 nonce 热修复

2026-09-23 用户报告线上登录后显示通用失败提示。发现授权码兑换时未向 MSAL 6 传入发起流程中保存的 `nonce`；该 SDK 在 ID Token 含 nonce 而缺少预期值时拒绝响应。已在 `acquireTokenByCode` 传入保存值，保留原有 state、浏览器绑定、PKCE、租户、用户及令牌声明校验，不扩大权限。

提交 `00f6b59` 已推送并发布，镜像 `20260923-auth-00f6b59`，摘要 `sha256:f69fab1b1e60490018acce37e07f4bafa4f337b673ce746f448747b93f159c8e`。新增真实 MSAL 客户端配合离线网络响应的回归，复现缺少 nonce 时 `nonce_mismatch`、传入后成功；不再仅模拟 SDK 方法。4 项认证测试在本地及生产容器内均通过，构建和 lint 通过。线上会话入口、匿名拒绝、Entra 授权码跳转及数据库检查通过，两个原有项目保留，身份与网络不变；实际账号登录需用户重新从首页发起确认，旧授权码不可重用。

### Codex Studio、混合认证与独立工作区

2026-09-23 已在现有东南亚站点发布。认证及 Markdown 提交 `d85511e`、产品改名提交 `fb079e3` 均已推送。镜像标签 `20260923-auth-fb079e3`，摘要 `sha256:fd992ee942d72e4c5e2efd6523a58d78ae3b2ad2811530636b3dfa4846eddf81`。登录页、默认工作区标题、浏览器标签、PWA 主屏幕名称和离线页统一为 Codex Studio；基础设施标识和模型名不改。

- 官方 MSAL Node 服务端授权码 + PKCE 登录与 Argon2id 本地密码登录并存，应用自行管理会话，Web App Easy Auth 已关闭。配置的 Entra 管理员在设置中创建或明确重置本地账号；无公开注册，不在聊天或仓库传递密码。Entra 与本地账号默认是独立身份。
- API 强制项目所有权，覆盖对话、素材、下载、任务操作和事件流。管理员也不能查看其他用户项目。原有两个共享项目及数据保留，但未分配所有者，因此对新登录账号隐藏。Codex 继续按项目分开 thread/工作目录，模型凭据与串行执行进程共享。
- 助手正文支持安全 Markdown/GFM；原始 HTML 和外部图片自动加载关闭，表格/代码在手机内滚动。复制保留原文，Process 不加工。

29 项后端、30 项手机浏览器测试通过，改名后另跑两个尺寸共 4 项登录/PWA 检查；构建、lint 及生产容器内 3 项认证测试通过。生产容器使用 Node 原生 TypeScript 类型剥离，不安装开发依赖 `tsx`。线上核验匿名及伪造平台身份头访问私有 API 均返回 401，Entra 跳转使用正确回调和 S256，页面及 PWA 新名称正确。真实 Entra 用户登录和新版本下的模型调用仍待实际使用验证。

停启传播阶段曾出现平台 403 页面及连接重置，两次验证失败后恢复旧镜像和认证。修正有界就绪检查后成功发布；02:37:02 UTC 新镜像拉取完成，02:37:14 Blob 写入/读取/删除验证成功且迁移 0 文件，02:37:17 启动探针成功。发布前后 SQLite 完整性均为 `ok`、待处理任务为 0、项目数为 2；托管身份及 VNet 配置保持不变。实际切换保留旧 Entra 回调及隐式登录设置以支持旧版本恢复，新应用只使用授权码流程。

日本东 A100 SKU 访问申请已提交，等待审批；用户已明确暂停删除和迁移。此次没有删除现有资源、重建 VM、挂载新盘、下载模型或启动 GPU，512 GiB 数据盘及 ComfyUI 重建待后续授权执行。以下发布及 Easy Auth 描述属于历史记录。
...
2026-09-22 用户要求允许所有已获应用登录授权的用户控制 GPU VM，已移除独立 VM 管理员白名单。2026-09-23 起改由应用内会话授权：获准的 Entra 用户及管理员创建的本地账号均可使用 VM 控制，匿名请求被拒绝；Entra 登录仍校验租户和允许用户。
